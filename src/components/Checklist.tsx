    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="space-y-2">
        <AnimatePresence initial={false}>
          {sorted.map((item) => (
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -16 }} className="glass flex items-center gap-3 rounded-2xl p-3.5">
              <button
                onClick={() => toggle(item)}
              ...
              <span className={`flex-1 ${item.done ? "text-muted line-through opacity-70" : "text-fg"}`}>{item.text}</span>

...
| หน้า | รายละเอียด |
|------|------------|
| 🏠 **Dashboard** | ภาพรวมรายวัน — XP, สตรีค, งานใกล้ครบกำหนด, กิจกรรม/นัดหมาย (รวม Google Calendar), ปุ่มลัดเกม |
| 📅 **ตารางเรียน** | ตารางเรียนรายสัปดาห์ แยกสีวิชา + **ชื่อหนังสือเรียน** + แผง **งาน/กิจกรรมสัปดาห์นี้** (ดึงจากงาน + ปฏิทิน + Google) |
| 📝 **งาน/การบ้าน** | ติดตามการบ้าน/งาน พร้อมวันครบกำหนดและสถานะเสร็จ |
| 🗓 **ปฏิทิน** | ปฏิทินรายเดือนเต็มจอแบบ Google Calendar + เลือกแหล่งข้อมูล (Local / Google / งาน) + ซิงก์ Google Calendar (iCal/ICS) + บันทึกลง Google ได้ |
| ⏱ **โฟกัส (Pomodoro)** | จับเวลา Pomodoro + เสียง Ambient + สถิติรายวัน |
| 🃏 **แฟลชการ์ด** | สร้างสำรับการ์ด ทบทวนด้วย Spaced Repetition (SM-2) + จัดการการ์ด (ดู/แก้/ลบ) เข้าได้เสมอ |
...
| 🎮 **เกมการเรียน** | 8 มินิเกมทบทวนความรู้ + Gamification (ปรับตามระดับนักเรียน) |

> 🔁 **CRUD ครบทุกหน้า** — ทุกรายการ (คาบเรียน, งาน, กิจกรรมในปฏิทิน, การ์ด, สำรับ, โน้ต, เช็กลิสต์, เกรด) **เพิ่ม / แก้ไข / ลบ** ได้ครบ

### 🎮 8 เกมทบทวน (Learning Games)

| เกม | รายละเอียด |
|-----|------------|
| 🪢 **แฮงแมน** | ทายตัวอักษรทีละตัวจากคำศัพท์ ก่อนหุ่นถูกแขวน |
| 🃏 **จำไพ่** | พลิกหาคู่คำ-ความหมายให้ครบ (Memory Cards) |
| 🔢 **จับคู่ตัวเลข** | จับคู่โจทย์คณิตกับคำตอบให้ครบ — โจทย์ปรับตามระดับ (บวกลบ → log/√/กำลัง) |
...
#### 🎓 ระดับนักเรียน (Student Levels)

เกมคำศัพท์/ตัวเลขเลือก **ระดับนักเรียน** เพื่อปรับความยากของคำและโจทย์ให้เหมาะกับวัย:

| ระดับ | คำอธิบาย |
|-------|----------|
| 🌱 ประถมต้น | ป.1–3 · บวกลบ 1–10, คำง่าย |
| 🌿 ประถมปลาย | ป.4–6 · คูณหาร, คำศัพท์พื้นฐาน |
| 📗 ม.ต้น | ม.1–3 · พีชคณิต, ศัพท์วิชาการ |
| 📘 ม.ปลาย | ม.4–6 · แคลคูลัส, ศัพท์ขั้นสูง |
| 🎓 ป.ตรี | มหาวิทยาลัย · ระดับยากสุด |
...
- นัดหมายจาก Google แสดงสี 🟢 เขียว แยกจากนัดหมายที่กรอกเอง (ม่วง) และงาน (ส้ม)
- **เลือกแหล่งข้อมูล** ด้วย checkbox — Local / Google / งาน
- **บันทึก + Google** — สร้างกิจกรรมแล้วเปิดหน้า Google Calendar พร้อมข้อมูลกรอกล่วงหน้า
- แก้ไข/ลบกิจกรรมได้ทุกแหล่ง (กิจกรรม Google แก้ในแคชแอป — ซิงก์ใหม่จะถูกเขียนทับ)
...
|--------|-------|----------|
| 🔤 คำศัพท์ | **~830 คำ** | ชีวิตประจำวัน / ภาษาอังกฤษ / วิชาการ / ธุรกิจ / วิทยาศาสตร์ / เทคโนโลยี / การแพทย์ (แต่ละหมวด ~95–180 คำ) |
...
แต่ละคำมีระดับความยาก (ง่าย/กลาง/ยาก) ที่จับคู่กับ **ระดับนักเรียน** อัตโนมัติ — ประถมเห็นคำพื้นฐาน (apple, cat, sun...) ส่วน ป.ตรี เห็นคำขั้นสูง
...
| State Management | React Context + `usePersistentState` (localStorage, ไม่มี backend) |
...
2. เลือกเกม → ตั้งค่า: **ระดับนักเรียน** (คุมความยาก) + หมวดคำศัพท์ หรือหัวข้อสูตร
...
│   │   │   ├── EmptyPool.tsx           # fallback เมื่อหมวด/ระดับไม่มีคำ
│   │   │   ├── Hangman.tsx
│   │   │   ├── MemoryCards.tsx
│   │   │   ├── NumberMatch.tsx         # จับคู่ตัวเลข (โจทย์ตามระดับ)
│   │   │   ├── WordScramble.tsx
│   │   │   ├── QuizFormulas.tsx
│   │   │   ├── WordMatch.tsx
│   │   │   ├── SentenceOrder.tsx
│   │   │   └── SpeedMath.tsx
│   │   ├── Dashboard.tsx       # หน้าหลัก + ลัดเกม + กิจกรรม/นัดหมาย
│   │   ├── Schedule.tsx        # ตารางเรียน + หนังสือ + งานสัปดาห์นี้
│   │   ├── Exams.tsx           # ปฏิทิน + Google Calendar sync (CRUD)
│   │   ├── Flashcards.tsx      # แฟลชการ์ด + SM-2 + จัดการการ์ด
│   │   ├── Games.tsx           # Hub เกม + ระดับนักเรียน
│   │   ├── Focus.tsx           # Pomodoro
│   │   └── ...
│   ├── data/
│   │   └── gameContent.ts      # คำศัพท์ ~830 / สูตร 151 / ศัพท์วิทย์ 216
│   ├── store.tsx               # React Context + usePersistentState (localStorage)
